  ...
  function init() {
    ...
    (function initWeeklyPlanDrawer() {
      ...
      function buildWeeklyPrompt() {
        const exclusionList = buildExclusionList();
        const exclusionSection = exclusionList.length > 0
          ? `\n${'━'.repeat(50)}\n기 게재 이야기 — 인물·소재·서사 패턴 모두 재사용 금지\n${'━'.repeat(50)}\n${exclusionList.slice(0, 30).map(t => '  • ' + t).join('\n')}\n\n위 이야기들과 다음 요소가 겹치면 채택하지 않습니다:\n  - 동일 인물 또는 해당 인물의 다른 에피소드\n  - 유사 주제 (예: 기 게재된 '과학자의 우연한 발견'이 있다면 같은 패턴 불가)\n  - 유사 서사 구조 (예: 이미 '거절이 선한 결과로', '혼자의 저항이 운동으로'를 썼다면 같은 틀 불가)\n` : '';
...
【금지 패턴 — 아래 유형은 채택하지 않습니다】
✗ TED 강연·교과서·명언집·유명 다큐에 이미 실린 에피소드
✗ 에디슨의 실패, 처칠의 명언, 스티브 잡스의 결단처럼 '아는 사람의 아는 이야기'
✗ 역경→극복→성공의 일반 공식
✗ "알고 보니 사실 ~이었다"는 단순 반전 (사실 정보만 추가되는 구조)
✗ "이 인물은 사실 평범했다" 또는 "이 인물은 사실 위대했다" 단일 메시지

【필수 조건】
✓ 제목을 들었을 때 "어, 그게 뭐야?"가 나올 것 — 이야기의 전제 자체가 낯설어야 함
✓ 이야기 중반의 전환점이 앞의 모든 내용을 다시 읽게 만들어야 함
✓ 5편 중 최소 2편은 한국 소재 또는 비서구권(아시아·아프리카·중남미) 이야기
✓ 각 이야기는 실존 인물·사건·기록에 근거한 팩트 기반

【반전 구조 가이드 — 아래 유형 중 하나를 명확히 가질 것】
① 행위자의 의도와 결과가 정반대인 경우 (원했던 것이 아닌 것을 만든 경우)
② 누군가의 '실패'가 훨씬 나중에 가장 중요한 것이 된 경우
③ 작고 사소한 것이 예상 불가능한 방식으로 거대한 것을 만든 경우
④ 알려진 사실의 '이면'이 드러나면서 이미 알던 것이 달리 보이는 경우
⑤ 이야기의 진짜 주인공이 끝에 가서야 밝혀지는 경우
...
=== 월요일 ===
【후보 1】
[제목] 짧고 기억에 남는 구절 (인물명 포함 가능)
[소재·출처] 실존 인물/사건/작품명, 연도
[주요 IHE] 통찰 또는 유머 또는 감동
[IHE 이유] 이 이야기가 왜 통찰/유머/감동인지 1문장
[개요] 이야기 흐름을 담은 4–6문장 스케치 (처음·중간·반전·끝)
[반전 포인트] 이야기의 핵심 전환점을 한 줄로 — 반드시 구체적으로
